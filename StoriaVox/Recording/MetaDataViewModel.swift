...
class MetaDataViewModel: ObservableObject {
    ...
    @Published var storyTitleError: String? = nil
    ...
    @Published var uploadedStoryId: Int? = nil
    ...
    init(audioURL: URL? = nil) {
        ...
        self.categories = UserDefaultsManager.shared.savedCategories ?? []
    }
...
extension MetaDataViewModel {
    func publishStory() {
        guard let audioURL = audioURL else {
            BannerHandler.shared.showErrorBanner(title: "Error", message: "No audio file selected", isAutoHide: true)
            return
        }
        guard let loggedInUser = UserDefaultsManager.shared.loggedInUser else {
            return
        }
        ...
        do {
            ...
            StoryService.shared.uploadStory(
